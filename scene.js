/*
 * Signal Pulse — встроенная комикс-сцена анализа.
 * Карточка файла + карточка анализа + живой диалог Дофамин/Опыт + футер.
 * Юмор от контраста: обезьяна рвётся жать кнопку, старик осаживает.
 */
(function () {
  "use strict";

  var DOP = "dop"; // Дофамин — импульс, обезьяна
  var OPY = "opy"; // Опыт — холодный ветеран
  ...
  var LIB = {
    ru: {
      ...
      intro: [
        [{who:DOP,text:"Оу-оу, смотри!\nДвижуха пошла 🔥"},{who:OPY,text:"Дыши, чемпион.\nСначала посмотрим."}],
        [{who:DOP,text:"Ого, какой импульс!\nЩас как заскочим 🚀"},{who:OPY,text:"Опять твоё «щас».\nЯ это кино уже видел."}],
        [{who:DOP,text:"Ты глянь на свечу!\nОна же конфетка!"},{who:OPY,text:"Красивая свеча ≠ вход, обезьяна."}]
      ],
      middle: [
        [{who:OPY,text:"Верхняя зона сильная.\nТам уже разворачивали."},{who:DOP,text:"Да в этот раз по-другому!"}],
        [{who:DOP,text:"Все входят, а мы стоим 😤"},{who:OPY,text:"«Все входят» — так и сливают.\nЖдём."}],
        [{who:DOP,text:"Кнопка прям сама тянется 👆"},{who:OPY,text:"Убери палец. Рано."}],
        [{who:OPY,text:"Смотрю объёмы. Не части."},{who:DOP,text:"Я спокоен! …почти."}]
      ],
      up: [
        [{who:DOP,text:"Ого, смотри какой импульс!\nЩас как заскочим 🚀\nСвеча просто конфетка!"},{who:OPY,text:"Не спеши, обезьяна.\nВерхняя зона сильная.\nСначала подтверждение."},{who:DOP,text:"Ну ладно…\nНо если пойдёт —\nне говори, что не звал 😎"},{who:OPY,text:"Если пойдёт — я первый зайду.\nНо по плану. Всегда."}],
        [{who:DOP,text:"Подтвердило! Го-го-го! 🚀"},{who:OPY,text:"Вот теперь можно.\nВидишь? Терпение — тоже скилл."}]
      ],
      down: [
        [{who:DOP,text:"Ну чего тормозишь!\nУйдёт же 😭"},{who:OPY,text:"Пусть уходит.\nТаких «уйдёт» я видел тысячу."},{who:DOP,text:"А вдруг это оно?!"},{who:OPY,text:"«Вдруг» — не стратегия, чемпион.\nПодтверждения нет."}],
        [{who:DOP,text:"Рука сама тянется войти…"},{who:OPY,text:"Значит, сегодня не входим.\nНе на эмоции."}]
      ],
      none: [
        [{who:DOP,text:"Смотри, дёрнулось!\nМожет, наш момент? 👀"},{who:OPY,text:"Может. А может, ты просто хочешь.\nПроверяем."},{who:DOP,text:"Ну хоть чуть-чуть зайдём?"},{who:OPY,text:"Сегодня наш вход — подождать.\nЭто тоже вход."}],
        [{who:DOP,text:"Скучно же просто стоять 😑"},{who:OPY,text:"Скучно — это дёшево.\nСлитый депозит — дорого."}]
      ]
    },
    en: {
      ui: {
      ...
      intro: [
        [{who:DOP,text:"Whoa, look!\nIt's moving 🔥"},{who:OPY,text:"Breathe, champ.\nLet's look first."}],
        [{who:DOP,text:"What an impulse!\nLet's jump in 🚀"},{who:OPY,text:"Your «right now» again.\nSeen this movie."}]
      ],
      middle: [
        [{who:OPY,text:"Upper zone is strong.\nGot rejected there before."},{who:DOP,text:"But this time is different!"}],
        [{who:DOP,text:"Everyone's in but us 😤"},{who:OPY,text:"«Everyone's in» is how they blow up.\nWait."}]
      ],
      up: [
        [{who:DOP,text:"Look at that impulse!\nLet's jump in 🚀\nThe candle is candy!"},{who:OPY,text:"Slow down, monkey.\nUpper zone is strong.\nConfirmation first."},{who:DOP,text:"Fine…\nBut if it runs —\ndon't say I didn't call it 😎"},{who:OPY,text:"If it runs, I'm first in.\nBut by the plan. Always."}],
        [{who:DOP,text:"Confirmed! Go go go! 🚀"},{who:OPY,text:"Now we can.\nSee? Patience is a skill too."}]
      ],
      down: [
        [{who:DOP,text:"Why are you stalling!\nIt'll leave 😭"},{who:OPY,text:"Let it leave.\nSeen a thousand of these."},{who:DOP,text:"But what if it's the one?!"},{who:OPY,text:"«What if» isn't a strategy, champ.\nNo confirmation."}]
      ],
      none: [
        [{who:DOP,text:"It twitched!\nMaybe our moment? 👀"},{who:OPY,text:"Maybe. Or maybe you just want it.\nWe check."},{who:DOP,text:"Can't we go in a little?"},{who:OPY,text:"Today our entry is to wait.\nThat's an entry too."}]
      ]
    }
  };
  ...
  function restoreVoicePref() {
    var saved = null;
    try { saved = localStorage.getItem("ps_voice"); } catch (e) {}
    if (saved === "1") {
      VOICE.on = true;
      var unlock = function () {
        unlockAudioPool();
  ...
  function ensureVoiceBtn() {
    ...
    var b = document.getElementById("psVoiceBtn");
    if (!b) {
    ...
    b.textContent = VOICE.on ? "\u{1F50A}" : "\u{1F507}";
  ...
  function injectStyle() {
    ...
    css.textContent = [
      ...
      ".ps-file-pair{font-size:15px;font-weight:800;color:var(--text);letter-spacing:.01em;}",
      ...
      '.ps-analysis::before{content:"";position:absolute;left:-20%;right:-20%;top:-70px;height:200px;background:radial-gradient(60% 100% at 35% 0%,rgba(90,166,255,.18),rgba(255,92,114,.08) 48%,transparent 72%);pointer-events:none;}',
      ...
      ".ps-an-bar > span{position:absolute;left:0;top:0;height:100%;width:0;border-radius:99px;background:linear-gradient(90deg,var(--ps-opy),#8E7BFF);box-shadow:0 0 18px rgba(120,150,255,.6);transition:width .3s ease;}",
      ...
      ".ps-thread{display:flex;flex-direction:column;gap:22px;margin-top:22px;}",
      ".ps-row{display:flex;align-items:flex-start;gap:12px;animation:psIn .42s cubic-bezier(.22,.9,.32,1) both;}",
      ".ps-row.dop{flex-direction:row;}",
      ".ps-row.opy{flex-direction:row-reverse;}",
      ".ps-av{flex:0 0 auto;display:flex;flex-direction:column;align-items:center;width:64px;}",
      ".ps-face{width:60px;height:60px;border-radius:18px;overflow:hidden;border:1px solid var(--ps-brd);background:var(--card);}",
      ".ps-row.dop .ps-face{width:62px;height:62px;border-color:rgba(255,92,114,.5);box-shadow:0 0 22px rgba(255,92,114,.5);}",
      ".ps-row.opy .ps-face{border-color:rgba(90,166,255,.45);box-shadow:0 0 20px rgba(90,166,255,.45);}",
      ".ps-face img{width:100%;height:100%;object-fit:cover;display:block;}",
      ".ps-name{margin-top:8px;font-size:10px;font-weight:800;letter-spacing:.12em;text-align:center;}",
      ".ps-row.dop .ps-name{color:var(--ps-dop);text-shadow:0 0 10px rgba(255,92,114,.5);}",
      ".ps-row.opy .ps-name{color:var(--ps-opy);text-shadow:0 0 10px rgba(90,166,255,.45);}",
      ".ps-bubble{max-width:78%;padding:13px 16px;border-radius:20px;font-size:15px;line-height:1.5;color:var(--text);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);box-shadow:inset 0 1px 0 rgba(255,255,255,.05),0 14px 34px -22px rgba(0,0,0,.9);}",
      ".ps-row.dop .ps-bubble{border:1px solid rgba(255,92,114,.36);background:linear-gradient(180deg,rgba(255,70,92,.15),rgba(120,20,40,.10));border-top-left-radius:7px;}",
      ".ps-row.opy .ps-bubble{border:1px solid rgba(120,150,200,.26);background:linear-gradient(180deg,rgba(34,44,72,.55),rgba(20,26,44,.45));border-top-right-radius:7px;}",
      ...
      ".ps-pause{align-self:center;color:var(--muted);font-size:20px;letter-spacing:7px;padding:2px 0;opacity:.7;animation:psIn .3s ease both;}",
      ".ps-foot{display:flex;align-items:center;gap:12px;margin-top:22px;color:var(--muted);font-size:13px;}",
